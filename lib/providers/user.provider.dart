...
class UserProvider extends ChangeNotifier {
  ...
  Future<void> login(String email, String password) async {
    try {
      ...
      if (response["code"] == 200) {
        ...
        checkLogin = true;
        SharedPre.setToken(token!);
        print("Test token2: $token");
      } else {
  ...
  void logout() {
    user = User(id: null, email: '', fullName: '', address: '', phone: '', dateOfBirth: '', genderName: '', avatar: '');
    token = ''; // Xóa token
    errorMessage = ''; // Xóa thông báo lỗi
    SharedPre.clearToken();
    notifyListeners();
  }

  Future<void> updateUser(User updatedUser) async {
    try {
      final response = await UserService.updateUser(updatedUser);
      if (response["code"] == 200) {
        user = updatedUser;
        notifyListeners();
      } else {
